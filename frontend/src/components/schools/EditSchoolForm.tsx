...
import { School, UpdateSchoolData } from '@/lib/types';
...
interface EditSchoolFormProps {
  school: School;
  onSubmit: (data: UpdateSchoolData) => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
}

export default function EditSchoolForm({ school, onSubmit, onCancel, isLoading }: EditSchoolFormProps) {
  ...
  useEffect(() => {
    ...
    setValue('country', school.country);
    setValue('zipCode', school.zipCode);
    setValue('phone', school.phone);
    setValue('email', school.email);
    setValue('website', school.website || '');
    setValue('isActive', school.isActive);
    setValue('academicYear', school.settings.academicYear);
    setValue('termSystem', school.settings.termSystem);
    setValue('gradeSystem', school.settings.gradeSystem);
    setValue('attendanceThreshold', school.settings.attendanceThreshold);
  }, [school, setValue]);

  const onFormSubmit = async (data: EditSchoolFormData) => {
    try {
      setSubmitError(null);
